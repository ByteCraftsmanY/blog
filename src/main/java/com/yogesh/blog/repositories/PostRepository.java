...
//
//    @Query(nativeQuery = true, value = "select * from posts where ((content like %?1% or excerpt like %?1% or title like %?1%) or id in (select post_id from post_tags where tag_id in (select id from tags where name = ?1))) and published_at >= ?2 and published_at <= ?3")
